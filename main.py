...
@app.route('/report/')
def report():
    donations = Donation.select(Donation.donor, fn.Sum(Donation.value).alias('total'), fn.Avg(Donation.value).alias('average')).group_by(Donation.donor)
...
def genemail():
    if request.method == 'POST':
        ...
        if donor is not None:
            ...
            return render_template('genemail.jinja2', error="That donor does not exist.")
    else:
...
def viewemail(donor):
    ...
    for donation in donations:
        ...
        count += 1
    return render_template('viewemail.jinja2', donor=donor, total=total, count=count)
